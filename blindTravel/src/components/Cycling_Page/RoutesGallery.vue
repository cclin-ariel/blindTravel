      ...
      <div class="row mx-auto box">
        <div
          ...
          class="
            ...
            justify-self-center
          "
          @click="toDetail(route.City, route.RouteName)"
        >
          <p class="title py-1 mt-2 text-bldGreen text-center fw-light fs-5">
            {{ route.City }} {{ route.RouteName }}
          </p>
          <div>
            <span class="pe-2">
              <svg
...
// import GoogleMap from "@/components/Cycling_Page/GoogleMap.vue";

export default {
  name: "RoutesGallery",
  components: { GoogleMap, Polyline },
  props: {
    updateCityList: { type: Array }, //from landing
    CityFromLanding: {
      type: String,
      default: "none",
    },
  ...
  beforeMount() {
    this.getCity(); //show on the gellery
  },
  mounted() {
    // console.log("mounted CityFromLanding", this.CityFromLanding);
    // console.log("mounted CityFromDetail", this.CityFromDetail);
    this.getCityRoutes(); //show on the gellery
  },

  methods: {
    getCity() {
      if (this.CityFromLanding !== "none") {
        this.cityRoutes = this.CityFromLanding;
        // console.log("this.CityFromLanding", this.CityFromLanding);
        this.getCityRoutes();
      } else if (this.CityFromDetail !== "none") {
        // console.log("this.CityFromDetail", this.CityFromDetail);
        this.cityRoutes = this.CityFromDetail;
        this.getCityRoutes();
      }
    },
    getCityRoutes() {
      ...
      //city from landing
      vm.enCity = this.cityTranslateing(vm.cityRoutes);
      const cityRoutApi = `https://ptx.transportdata.tw/MOTC/v2/Cycling/Shape/${vm.enCity}?$top=10&$format=JSON`;
      ...
      axios
        ...
        .then((response) => {
          vm.cityRoutes = response.data;
          // console.log(vm.cityRoutes);
        })
    ...
    toDetail(city,routeNmae) {
      this.$router.push({
        name: "CyclingDetail",
        params: {
          city: city,
          routeNmae: routeNmae,
          cityList: JSON.stringify(this.updateCityList), // cityList from landing to detail
        },
      });
    }, //get marker of map
    getMarker(geo) {
      let CoordinatesPath = new Array();
      // console.log('123',geo);
      let x = new String(geo);
      // x = x.replace('MULTILINESTRING','');
      // console.log(x);
      let y = x.replace("MULTILINESTRING ((", "");
      y = y.replace("))", "");
      if (y.indexOf("),(" !== -1)) {
        y = y.split("),(")[0];
      }
      let z = y.split(",");
      z.forEach((Coordinate) => {
        Coordinate = Coordinate.split(" ");
        let CoordinateObj = {};
        CoordinateObj.lat = Number(Coordinate[0]);
        CoordinateObj.lng = Number(Coordinate[1]);
        // console.log("456", CoordinateObj);
        CoordinatesPath.push(CoordinateObj);
      });
    ...
    getCenter(geo) {
      let CoordinatesPath = { lat: "", lng: "" };
      let x = new String(geo);
      let y = x.replace("MULTILINESTRING ((", "");
      y = y.replace("))", "");
      let z = y.split(",");
      let a = z[0].split(" ");
      CoordinatesPath.lat = Number(a[0]);
      CoordinatesPath.lng = Number(a[1]);
      // console.log('zzzzzz',CoordinatesPath);
    ...
    cityTranslateing(chCity) {
      const vm = this;
      const result = vm.updateCityList.filter(
...
.title {
  border-bottom: #7ca23c 0.5px solid;
}
